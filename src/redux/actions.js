import {
  ...
  SET_CURRENT_ANSWER,
  SET_ANSWER_PROC_STATUS,
  HIGHLIGHT_TRUE_ANSWER,
  SET_NEXT_ROUND,
  SET_GAME_PROC_STATUS,
  SET_CURRENT_SOUND_PROPS,
  SET_CURRENT_SOUND_PROPS_AFTER_COMPARE,
  SET_ANSWERS_DISABLED_STATUS,
  SET_HELP_ITEM_USED_STATUS,
  SET_HELP_ITEM_SHOWN_STATUS,
  SET_CURRENT_HELP_ITEM_DATA,
  SET_SOUND_AVAILABILITY,
} from "./types";
...
export function highlightTrueAnswer(currentAnswer) {
  return {
...
export function setCurrentSoundProps(
  url,
  playFromPosition,
  autoLoad = false,
  autoPlay = false,
...
export function setCurrentSoundPropsAfterCompare(
  rightAnswerSoundUrl,
  wrongAnswerSoundUrl
) {
  return {
...
export function asyncSetCurrentSoundProps(
  checkingSoundUrl,
  rightAnswerSoundUrl,
  wrongAnswerSoundUrl
) {
  return async function (dispatch) {
    ...
    await setTimeoutPromise(3000);
    dispatch(
      setCurrentSoundPropsAfterCompare(rightAnswerSoundUrl, wrongAnswerSoundUrl)
    );
  };
}
  ...
  return async function (dispatch) {
    ...
    switch (helpItem) {
      ...
      case "audienceHelp":
        ...
        const b_perc = Math.round(
          Math.random() * (100 - (a_perc + rightAnswerPerc))
        );
      ...
      case "changeQuestion":
        fetch(
          ...
          .then((data) => {
            ...
            helpItemData.variants.splice(
              rightAnswerIndex,
              0,
              helpItemData.correct_answer
            );
